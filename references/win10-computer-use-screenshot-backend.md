...
- Live install on Windows 10 build `19045` stored the original at `.codex\backups\computer-use-helper\26.908.4834.0-sky-0.6.32-BAD605EF\codex-computer-use.exe.original`, then verified the complete patched hash; the read-only patcher reports `State: patched` with `EndToEndValidatedDesktopVersion` equal to `CurrentDesktopVersion` (`26.908.4834.0`).

This profile is an exact input/output hash pair derived from the validated `0.6.26` guarded-code layout. Real Computer Use capture validation on Windows 10 build `19045` completed end-to-end through the official `@oai/sky 0.6.32` runtime driven from short-lived external `node.exe` processes (separate processes per batch, stable non-minimized targets):
...
| Dynamic capture | Frames before and after a `type_text` content change produced two distinct SHA-256 values (`356a6cb2...` -> `23265d49...`), proving live frames rather than a cached first frame. |
...
Frame file bytes and SHA-256 values above were verified programmatically (valid JPEG media type, expected dimensions, deterministic hashes, and a real content change under input); the operator performed the visual frame-content check on the written `.img` files.
...
The regression validates the exact original and candidate hashes, unknown-hash rejection, and the platform guard using a temporary helper copy. On Windows 11 it must reject installation and leave that copy unchanged; it does not perform or claim Windows 10 capture acceptance.
